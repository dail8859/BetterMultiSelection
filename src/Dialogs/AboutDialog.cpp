...
INT_PTR CALLBACK abtDlgProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam) {
	switch(uMsg) {
		case WM_INITDIALOG:
			...
			Edit_SetText(GetDlgItem(hwndDlg, IDC_VERSION), TEXT("BetterMultiSelection v") VERSION_TEXT TEXT(" ") VERSION_STAGE TEXT(" ") BITNESS);
			return true;
		case WM_COMMAND:
			switch(LOWORD(wParam)) {
				case IDOK:
					...
					return true;
				case IDC_GITHUB:
					ShellExecute(hwndDlg, TEXT("open"), TEXT("https://github.com/dail8859/BetterMultiSelection/"), NULL, NULL, SW_SHOWNORMAL);
					return true;
			}
		case WM_DESTROY:
			DestroyWindow(hwndDlg);
